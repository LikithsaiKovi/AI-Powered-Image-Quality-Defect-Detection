...
The model is a multi-output Random Forest: it predicts independent binary probabilities for `blur`, `underexposure`, `overexposure`, `noise`, and `defect`. Inputs are eleven interpretable image features extracted with OpenCV.

## Data generation

Start with clean source images in `data/clean/`. The training script creates controlled transformations: Gaussian blur, reduced/increased exposure, additive Gaussian noise, and localized lines/blocks for visual defects. It also makes multi-issue examples. Every generated variant inherits labels from the transformation that created it.
...
The training command prints validation F1 for each label. Record this output in the final submission after training with real, unseen source images. Do not report the Docker demo bootstrap model as an evaluation result.
